...
After completing task work and its required quality gates:
- review `git status`
- commit all task-related changes with a concise imperative message
- do not push or sync without explicit user approval
...
- **Commit-on-completion (default)**: Use `bd` for task tracking. After quality gates, commit completed task changes before handoff. Do not push or run Dolt remote sync unless explicitly asked.
- **Minimal**: Keep tool instruction files as pointers to `bd prime`; use the same commit-on-completion policy unless active instructions say otherwise.
...
4. **Handle git/sync by active profile**:
   ```bash
   # Commit-on-completion/minimal/default: commit verified task changes; do not push or sync.
   git status
...
- Commit completed task changes after quality gates; do not push or sync without clear authority from the active profile or the current user request.
